...
function triggerBackupDownload() {
    ...
    const backupData = {
        ...
        lists: state.appData.rawLists,
        tasks: state.appData.tasks // Object
    };
...
async function restoreBackup(data) {
    ...
    // 2. Restore Lists
    // First delete existing? Or just overwrite.
    // For a clean restore, deleting old collections is harder in client-side w/o cloud function, 
    // but we can overwrite known IDs. The user said "overwrite".
    // A true "overwrite" usually implies clearing old data. 
    // Given the constraints, let's just write the new data. 
    // If the user wants a clean slate, they should Reset App first.

    if (data.lists && Array.isArray(data.lists)) {
        data.lists.forEach(l => {
            batch.set(doc(db, "users", state.currentUser.uid, "lists", l.id), l);
        });
    }
    ...
    if (data.tasks) {
        Object.keys(data.tasks).forEach(tid => {
            batch.set(doc(db, "users", state.currentUser.uid, "tasks", tid), data.tasks[tid]);
        });
    }

    await batch.commit();
    Utils.showToast("Backup Restored!");
