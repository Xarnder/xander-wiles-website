import {
    SITE_LLM_MODELS,
    canUseSiteLocalLLM,
    getSiteLocalLLMModelLabel,
    runSiteLocalLLMChatCompletion
} from '/assets/js/local-llm.js';

const PRIMARY_MODEL_ID = SITE_LLM_MODELS.qwenTodo.id;
const BACKUP_MODEL_ID = SITE_LLM_MODELS.llamaTodo.id;
...
async function runModel(modelId, taskText, onStatus) {
    const reply = await runSiteLocalLLMChatCompletion(modelId, {
...
export function getLocalAIModelId() {
    return getSiteLocalLLMModelLabel(PRIMARY_MODEL_ID, BACKUP_MODEL_ID);
}
