# Tidal Breathing — adaptive breathing
...
Open `/pages/Tidal-Breathing-Exercise/` on the printed local origin. `npm run build` produces `dist/`; the root site's `build.js` builds and copies it to `deploy_out/pages/Tidal-Breathing-Exercise/`. Nothing is published by the app's build command.
