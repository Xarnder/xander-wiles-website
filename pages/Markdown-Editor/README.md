...
16. **Pinned / Finder / Settings** keep the open file in memory and save in the background. Tapping **Save** (default) shows a “Saving in progress” lock until Drive finishes; autosave never blocks the screen. You can turn the lock off in Settings after a warning. The unsaved-file prompt still appears when opening a different file or signing out.
