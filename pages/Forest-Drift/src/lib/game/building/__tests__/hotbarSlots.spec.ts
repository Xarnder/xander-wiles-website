...
describe('isCustomizablePlacementTool', () => {
	it('is true for windows, doors, beams, walls, and floor detailing', () => {
		...
		expect(isCustomizablePlacementTool('stairs')).toBe(false);
		expect(isCustomizablePlacementTool('torch')).toBe(false);
	});
});
