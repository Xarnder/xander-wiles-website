...
import { buildSlabGeometry, slabLocalPolygon } from './SlabGeometryBuilder';
...
export class SlabManager {
	...
	private buildEntry(definition: SlabDefinition, existing?: SlabEntry): SlabEntry | null {
		...
		const geometry = buildSlabGeometry(localPolygon, definition.localY, bottomLocalY);
	...
	/** Every placed slab's top (walkable) world Y at (worldX, worldZ) — a point can be above several stacked slabs, hence an array; see WorldSurfaceSampler for how the right one is chosen. */
	getTopSurfacesAt(worldX: number, worldZ: number): number[] {
		const point = { x: worldX, z: worldZ };
		const tops: number[] = [];
		for (const entry of this.slabs.values()) {
			if (pointInPolygon2D(point, entry.worldPolygon)) tops.push(entry.topWorldY);
		}
		return tops;
	}

	/** Every placed slab's underside world Y at (worldX, worldZ) — used to block upward movement into a ceiling from below. */
	getUndersidesAt(worldX: number, worldZ: number): number[] {
		const point = { x: worldX, z: worldZ };
		const undersides: number[] = [];
		for (const entry of this.slabs.values()) {
			if (pointInPolygon2D(point, entry.worldPolygon)) undersides.push(entry.bottomWorldY);
		}
		return undersides;
	}
