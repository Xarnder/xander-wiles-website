document.addEventListener('DOMContentLoaded', () => {
    ...
    const setupUI = () => {
        ...
        for (let i = 0; i < numImages; i++) {
            ...
            splitWrapper.className = splitWrapperClass;
            ...
            magnifyWrapper.className = magnifyWrapperClass;
    ...
    const processAndLoadImage = (file, index) => {
        ...
        const processFileBlob = (fileBlob) => {
            ...
            reader.onload = (e) => {
                ...
                tempImg.onload = () => {
                    imagesData[index] = { src: imageUrl, naturalWidth: tempImg.naturalWidth, naturalHeight: tempImg.naturalHeight, element: tempImg, filename: file.name };
        ...
        if (fileType === 'heic' || fileType === 'heif') {
            ...
            heic2any({ blob: file, toType: "image/jpeg", quality: 0.9 })
                ...
                .catch(err => {
                    ...
                    showErrorPopup('HEIC Conversion Failed', 'There was an error converting the HEIC file. It may be corrupted or in an unsupported format.');
                });
        } else {
    ...
    const updateMagnifier = (e) => {
        ...
        const eventForCalc = e || lastMagnifyEvent;
        ...
        const rawX = eventForCalc.clientX - sourceRect.left, rawY = eventForCalc.clientY - sourceRect.top;
        const correctedX = rawX - offsetX, correctedY = rawY - offsetY;
    ...
    zoomLevelSlider.addEventListener('input', () => {
        ...
        document.getElementById('magnify-loupe').style.width = `${renderedW / zoom}px`;
        document.getElementById('magnify-loupe').style.height = `${renderedW / zoom}px`;
        updateMagnifier();
    });
    ...
    const generateHeatmap = () => {
        ...
        setTimeout(() => {
            ...
            for (let i = 0; i < diffs.length; i++) {
                const normDiff = maxDiff > 0 ? diffs[i] / maxDiff : 0;
                const color = applyColormap(normDiff);
    ...
    const updateExportPreview = () => {
        ...
        canvasHeight = (baseHeight + labelHeight) * rows;
    ...
    const exportImage = () => {
        ...
        setTimeout(() => {
            ...
            link.href = dataUrl;
            link.download = `comparison-${new Date().getTime()}.${fileExtension}`;
            link.click();
    ...
    navLinks.forEach(link => {
        link.addEventListener('click', (e) => {
            ...
            if (targetView === 'export-view' && imagesData.some(d => d)) {
                updateExportPreview();
            }
    ...
    magnifyAspectRatioSelect.addEventListener('change', (e) => {
        ...
        if (width > height) {
            magnifyViewTool.classList.add('landscape-mode');
        } else {
            magnifyViewTool.classList.remove('landscape-mode');
        }
        magnifyPreviewContainer.style.paddingTop = `${(height / width) * 100}%`;
    ...
    exportFontSizeSlider.addEventListener('input', (e) => {
        fontSizeValueSpan.textContent = e.target.value;
        updateExportPreview();
    });
